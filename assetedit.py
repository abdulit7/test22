...
class AssetEditPage:
    def __init__(self, page: ft.Page, parent=None, asset_id=None, local_db=None):
        ...
        self.page.secret_key = "mysecret123"  # Set the secret key (optional, not used here)

        if self.asset_id:
            self.load_asset_data()

    def on_upload_complete(self, e: ft.FilePickerUploadEvent):
        # This method is no longer critical; keeping it for compatibility
        pass
    ...
    def handle_asset_image(self, e: ft.FilePickerResultEvent):
        print(f"handle_asset_image called with event: {e}, files: {e.files}")
        ...
        if self.attached_images:
            ...
            try:
                if file.path and os.path.exists(file.path):  # Works for both Windows and Android
    ...
    def handle_bill_image(self, e: ft.FilePickerResultEvent):
        print(f"handle_bill_image called with event: {e}, files: {e.files}")
...
#ft.app(target=lambda page: AssetEditPage(page).open_dialog())